...
app.get('*', (req, res) => {
    ...
    const link = protocol === 'https' ?
        `pler://${uuid}@${host}:${port}?path=${path}&security=tls&encryption=none&host=${host}&type=ws&sni=${host}#node-pler` :
        `pler://${uuid}@${host}:${port}?type=ws&encryption=none&flow=&host=${host}&path=${path}#node-pler`;
